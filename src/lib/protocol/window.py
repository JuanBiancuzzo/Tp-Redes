...
class Window:
    ...
    # Si el tamaño no cambia me mandaron un ack de uno que ya me ackearon, por
    # lo que se perdió el siguiente.
    def remove_acked_segments(self, ack_number) -> int:
        num_ack_remove = 0
        while len(self.window) > 0 and \
                self.window[0][0].header.seq_num < ack_number:
            self.window.popleft()
            num_ack_remove += 1
    ...
    def send_new_batch(self):
        ...
        while not self.window_is_full() and len(self.segments) > 0:
            ...
            self.logger.log(OutputVerbosity.VERBOSE,
                            f"Sent segment: {segment_to_send.header.seq_num}")

            try:
                self.socket.sendto(
                    segment_to_send.serialize(),
    ...
    def resend_oldest_segment(self):
        ...
        self.logger.log(OutputVerbosity.VERBOSE,
                        f"Re-sending segment: {oldest_segment.header.seq_num}")

        try:
            self.socket.sendto(
                oldest_segment.serialize(),
                self.receiver_address)
        except SocketError:
            raise ProtocolError.ERROR_SENDING_MESSAGE
